  ...
  <div class="p-4 sm:ml-64">
    ...
    <div class="p-2 border-2 border-gray-200 border-dashed rounded-lg mt-4">
      ...
      <div class=" px-5 py-3 text-gray-700 bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="flex justify-between items-center my-6">
          <h2 class="text-2xl font-bold leading-9 tracking-tight text-gray-900">Détails du promotion</h2>
        </div>
        <div class="flex justify-between items-center my-6">
          <h3 class="text-xl font-medium leading-9 tracking-tight text-gray-900">{{$promotion->nom_promotion}}</h3>
          <div class="flex items-center">
            <a href="{{route('admin.gestionPromotions.edit',$promotion->id)}}">
        ...
        <div class="flex flex-col md:flex-row gap-10 items-center">
          ...
          <div class="md:w-[160px] md:h-[160px] overflow-hidden md:rounded-full border flex-shrink-0">
            @if($promotion->photo_promotion !== NULL)
            <img id="promotionImage" class="w-full h-full object-cover cursor-pointer" src="{{asset('storage/'.$promotion->photo_promotion)}}" alt="promotion image">
            @else
            <img id="promotionImage" class="w-full h-full object-cover cursor-pointer" src="/images/defaultimage.jpg" alt="promotion image">
            @endif
          </div>
          ...
          <div class="flex flex-col md:flex-row gap-5 w-full justify-center md:justify-start">
            ...
            <div class="flex-1 space-y-4">
              {{-- promotion --}}
              <div>
                <p class="capitalize text-sm font-medium text-gray-900">Promotion</p>
                <p class="text-sm text-gray-500">
                  {{ $promotion->nom_promotion ?? 'N/A' }}
                </p>
              </div>
              ...
              <div>
                ...
                <p class="text-sm text-gray-500">{{$promotion->ville ?? 'N/A' }}</p>
              </div>

              {{-- Modèle --}}
              <div>
                <p class="capitalize text-sm font-medium text-gray-900">Lien promotion</p>
                <p class="text-sm text-gray-500">
                  <a  href="{{ $promotion->lien_promotion }}" target="_blank"><svg class="w-5 h-5 text-gray-500 transition duration-75 hover:text-gray-900" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="flex-1 space-y-4">
              ...
              <div>
                <p class="text-sm font-medium text-gray-900">Date debut</p>
                <p class="text-sm text-gray-500">{{$promotion->date_debut ?? 'N/A' }}</p>
              </div>

              {{-- Date d'achat --}}
              <div>
                <p class="text-sm font-medium text-gray-900">Date fin</p>
                <p class="text-sm text-gray-500">{{$promotion->date_fin ?? 'N/A' }}</p>
              </div>

              {{-- Date de dédouanement --}}
              <div>
                <p class="text-sm font-medium text-gray-900">Garage</p>
                <p class="text-sm text-gray-500">{{$promotion->garage->name ?? 'N/A' }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div id="imageModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-75 flex items-center justify-center">
      <div class="relative max-w-4xl w-full mx-auto">
        <img
          id="modalImage"
          src="{{ $promotion->photo_promotion !== NULL ? asset('storage/'.$promotion->photo_promotion) : '/images/defaultimage.jpg' }}"
          alt="Expanded Image"
